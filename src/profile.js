    ...
    return(
      <div className='wrapper3'>
        <div>
          ...
          <button className="newPWBtn" ref='newpassword'>Send nytt passord på mail</button>
          {activateBtn}
          {adminBtn}
        </div>
    ...
    for (let skill of selectValue) {
      skillService.getSkillInfo(skill.value, (result) => {  // Kjører gjennom alle kompetanser som er valgt i multiselect
        if (result.duration === 0) {
          ...
          this.dateInputList.push(<tr key={skill.value}><td> { skill.label } </td><td><b>Utløpsdato: </b></td><td><input type='date' ref={(ref) => this.refNr = ref} /></td></tr>);
